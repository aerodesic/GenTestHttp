...
# Connect to gentest and return the access path
def _connect_to_erver():
    ...
    while service == None:
        try:
            ...
            service = None
            syslog.syslog("Waiting for gentest server...")
            time.sleep(1)

    syslog.syslog('gentest server is up')

    return service

gentestservice = _connect_to_server()

SendMsg              = gentestservice.get_dbus_method('SendMsg',               _BUSNAME)   # post
GetStatus            = gentestservice.get_dbus_method('GetStatus',             _BUSNAME)   # get
GetValue             = gentestservice.get_dbus_method('GetValue',              _BUSNAME)   # get
SetValue             = gentestservice.get_dbus_method('SetValue',              _BUSNAME)   # put
ApplyValues          = gentestservice.get_dbus_method('ApplyValues',           _BUSNAME)   # put
HttpRequest          = gentestservice.get_dbus_method('HttpRequest',           _BUSNAME)   # post
NetManagerRequest    = gentestservice.get_dbus_method('NetManagerRequest',     _BUSNAME)   # varies
ResetRequest         = gentestservice.get_dbus_method('Reset',                 _BUSNAME)   # get
RebootRequest        = gentestservice.get_dbus_method('Reboot',                _BUSNAME)   # get
ShutdownRequest      = gentestservice.get_dbus_method('Shutdown',              _BUSNAME)   # get
